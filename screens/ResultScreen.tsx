...
export default function ResultScreen({ navigation, route }: Props) {
  ...
  // Use passed fraudScore; fall back to fraudData object if fraudScore not present
  const score = fraudScore >= 0
    ? fraudScore
    : Math.round(fraudData?.final_score ?? fraudData?.fraud_score ?? -1);

  const scoreAvailable = score >= 0;
  const scoreColor = scoreAvailable ? getScoreColor(score) : COLORS.gray;
  const riskLabel  = scoreAvailable ? getRiskLabel(score)  : 'UNKNOWN';
